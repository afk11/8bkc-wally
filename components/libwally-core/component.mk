...
build:
	...
	export CFLAGS="-fno-stack-protector ${CFLAGS}" && ./tools/autogen.sh && \
			./configure --enable-tests=no --enable-static=no --enable-shared=no \
			--with-asm=no --enable-lto=no --disable-swig-java --disable-js-wrappers \
			--disable-swig-python --disable-dependency-tracking --enable-static-precomputation=yes \
			--host=xtensa-esp32-elf --target=x86_64-pc-linux-gnu --build=x86_64-pc-linux-gnu && \
			$(MAKE) -o configure && \
